...
function M.swiftformat_args_flexible(ctx)
  ...
  if not found or found.mode == "local" then
    return { "--stdin-path", filename }
  end
  return { "--config", found.path, "--stdin-path", filename }
end

function M.swiftlint_args_flexible(ctx)
  local filename = ctx and ctx.filename
  if not filename or filename == "" then
    filename = vim.api.nvim_buf_get_name(0)
  end
